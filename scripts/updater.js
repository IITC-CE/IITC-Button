...
async function getUrl(url, variant, retry) {
  if (retry > 1) {
    ...
    try {
      await browser.runtime.sendMessage({'type': "showMessage", 'message': _('serverNotAvailableRetry', seconds.toString())});
    } catch {
...
async function showProgress(value) {
  try {
    await browser.runtime.sendMessage({'type': "showProgressbar", 'value': value});
  } catch {
    // If popup is closed, message goes nowhere and an error occurs. Ignore.
  }
}
...
async function checkUpdates(force) {
  ...
  if (local[channel+'_last_modified'] === undefined || local.last_check_update === undefined) {
    ...
    const time_delta = Math.floor(Date.now() / 1000)-update_check_interval-local.last_check_update;
    if (time_delta >= 0 || force) {
      clearTimeout(update_timeout_id); update_timeout_id = null;
      const last_modified = await getUrl(network_host[channel]+"/meta.json", "Last-Modified", true);
...
async function downloadMeta(local, last_modified) {
  const response = await getUrl(network_host[channel]+"/meta.json", "parseJSON", true);
  if (!response) return;
  ...
  const p_iitc = async () => {
    const iitc_code = await getUrl(network_host[channel]+"/total-conversion-build.user.js");
    if (iitc_code) {
      await save({
        'iitc_code': iitc_code
      })
...
async function checkExternalUpdates(force) {
  const local = await browser.storage.local.get([
    "channel",
...
async function updateExternalPlugins(local) {
  ...
  if (plugins_user) {
    ...
    const promises = Object.keys(plugins_user).map(async function(uid) {
...
async function managePlugin(uid, category, action) {
  let local = await browser.storage.local.get([channel+"_plugins_flat", channel+"_plugins_local", channel+"_plugins_user"]);
  ...
  if (action === 'on') {
    ...
    if (plugins_flat[uid]['user'] === false && plugins_local[uid] !== undefined ||
      ...
      let response = await getUrl(`${network_host[channel]}/plugins/${filename}`);
      if (response) {
        plugins_flat[uid]['status'] = 'on';
...
async function addUserScripts(scripts) {
  let local = await browser.storage.local.get([channel+"_categories", channel+"_plugins_flat", channel+"_plugins_local", channel+"_plugins_user"]);
  ...
  scripts.forEach(script => {
    ...
    plugins_user[plugin_uid] = Object.assign(meta, {'uid': plugin_uid, 'status': 'on', 'code': code});
